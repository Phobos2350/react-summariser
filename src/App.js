...
class App extends Component {
  ...
  render() {
    const { namedEnts, keyTerms, stats, summary, sentiment, isLoading, status } = this.state
    ...
    return new Promise((resolve, reject) => {
        if (urlRegex().test(text)) { 
            ...
            .then(unfluffed => {
                ...
                return resolve(JSON.stringify({ text: unfluffed, length: parseInt(length, 10) }))
            })
            .catch(e => console.error(e))
        } else {
            return resolve(JSON.stringify({ text: text, length: parseInt(length, 10) }))
        }
    })
}
